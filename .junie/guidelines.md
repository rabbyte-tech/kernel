...
## Project Structure Notes (Kernel)
- Local packages live in `packages/*/` and expose `manifest.php`.
- Vendor packages are discovered when their `composer.json` defines `extra.kernel.manifest`.
- Package discovery and registry live in `app/Services/Packages/`.
- Event dispatch/envelope logic lives in `app/Services/Events/`.
- MCP entries are synced from manifests into the `mcp_entries` table.
- Filament resources live in `app/Filament/Resources/`.
- Kernel scope is wiring only; business logic lives in packages.

## Kernel Platform Rules (LLM)
- Treat this repo as kernel-only: do not introduce product-specific domain models or UI.
- Packages must declare permissions, events, listeners, MCP entries, routes, and Filament resources in `manifest.php`.
- Listener wiring uses string keys; avoid cross-package class references.
- Event names are dot-case with version suffix (e.g. `invoice.created.v1`).
- Event dispatch returns a standard envelope: `event`, `id`, `occurred_at`, `actor`, `source`, `data`, `meta`.
- All state mutations must go through domain actions with centralized authorization.
- Permission checks must respect package status (disabled package permissions are not allowed).
- Package discovery must support both `packages/*/manifest.php` and vendor packages with `extra.kernel.manifest`.
